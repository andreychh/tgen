...
class Message(BaseModel):
    ...
    specific instances (e.g., a message containing a video sent to a big
    chat), the server might automatically schedule a message instead of
    sending it immediately. In such cases, this field will be 0 and the
    ...
    chat. The identifier may be reused for another ephemeral message after
    the message is deleted or expires.
    """ 
    ...
    reply_to_message fields even if it itself is a reply. If the message is
    a reply to an ephemeral message, then this field may be omitted.
    """ 
...
class ReplyParameters(BaseModel):
    ...
    ephemeral message. An ephemeral message may only be replied to within 15
...
class ChatJoinRequest(BaseModel):
    ...
    requests only. If present, then the bot must call
...
class BotCommand(BaseModel):
    ...
    """
    True, if the command sends an ephemeral message, which can be seen only
    by the sender of the message and the bot
    """
...
class InputRichMessage(BaseModel):
    ...
    rich message formatting options for more details. Use media field to
...
class RichBlockThinking(BaseModel):
    ...
    https://t.me/addemoji/AIActions for examples of custom emoji that are
...
class InputRichBlockMap(BaseModel):
    """
    A block with a map, corresponding to the custom HTML tag <tg-map>. The
    map's width and height must not exceed 10000 in total. The width and
...
# A media element embedded in a rich message.
type InputRichMedia = (
        InputMediaAnimation
        | InputMediaAudio
        | InputMediaPhoto
