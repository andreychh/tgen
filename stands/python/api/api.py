...
class EphemeralMessageParameters(BaseModel):
    ...
    """Identifier of the user who will receive the message. It is not
    guaranteed that the user will receive the message, especially if
    they are offline. See here for more details.
    """
    ...
    """Pass True if the ephemeral message must be shown in place of the
    original message. Must be False for callback queries from ephemeral
    messages, which must be edited using regular editEphemeralMessage…
    methods.
    """
...
class MessageGenerationStopped(BaseModel):
    """This object describes an update about a user stopping message
    generation.
    ...
    message_thread_id: int | None = None
    """Unique identifier of the message thread in which the message is
    generated
    """
...
class CommunityChatJoined(BaseModel):
    """Describes a service message about a chat being joined by a user from
    a community.
...
class CommunityChatRemoved(BaseModel):
    """Describes a service message about a chat or a bot being removed from
    a community. Currently holds no information.
...
class ReplyKeyboardMarkup(BaseModel):
    ...
    """Pass True if the reply interface must be shown to the user, as if
...
class InlineKeyboardMarkup(BaseModel):
    ...
    force_reply: bool | None = None
    """Pass True if the reply interface must be shown to the user, as if
    they had manually selected the bot's message and tapped 'Reply'. The
    value of the field can't be changed when the inline keyboard is
    edited.
    """
...
class InlineKeyboardButton(BaseModel):
    ...
    as a replacement for the Telegram Login Widget. Not supported for
    ephemeral messages.
    """
...
class LoginURL(BaseModel):
    ...
    used to automatically authorize a user. It serves as a great
    ...
    """Username of a bot, which will be used for user authorization; not
    supported in RichMessageButton. See Setting up a bot for more
...
class ForceReply(BaseModel):
    ...
    force_reply: bool
    """Shows reply interface to the user, as if they had manually selected
...
class ChatAdministratorRights(BaseModel):
    ...
    """True, if the administrator can manage chat welcome messages or
...
class SendMessageMethod(BaseModel):
    ...
    """A JSON-serialized object containing the parameters of the ephemeral
    message to send
    """
...
class SendVideoNoteMethod(BaseModel):
    """Use this method to send a rounded square MPEG4 video of up to 1
    minute long. On success, the sent Message is returned.
...
class SendMessageDraftMethod(BaseModel):
    ...
    drafts with the same identifier are animated. Otherwise, the draft
    is replaced without animation.
    """
    ...
    """Pass True to show the user a button to stop further drafts. The bot
    will receive an Update “stopped_message_generation” if the user
    presses the button.
    """

    keep_on_stop: bool | None = None
    """Pass True to keep the draft in the chat when the button is pressed.
    The draft will still disappear after a short time or if the bot
    sends a message. To fully preserve the partial draft, the bot should
...
class PromoteChatMemberMethod(BaseModel):
    ...
    """Pass True if the administrator can manage chat welcome messages or
    directly send them in the case of bots
    """
...
class EditMessageTextMethod(BaseModel):
    ...
    Direct upload of new files and explicit upload of files by a URL
...
class EditEphemeralMessageTextMethod(BaseModel):
    """Use this method to edit an ephemeral text or rich message. Note that
...
class InputRichMessage(BaseModel):
    ...
    using tg://photo?id=, tg://video?id=, tg://document?id=, and
    tg://audio?id= links
    """
...
class RichMessageButton(BaseModel):
    """This object represents a button in a RichMessage. Exactly one of the
    fields other than text and style must be used to specify the type of
    the button.
    ...
    text: RichText
    """Text of the button. May contain only plain text, RichTextCustomEmoji
    and RichTextDateTime entities.
    """

    style: str | None = None
    """Style of the button. Must be one of “danger”, “success”, “primary”,
    or “link” (the button is shown as a regular link without borders).
    Apps may use theme-specific colors for the button background and
    text based on the style. The style “link” is allowed only for
    callback buttons.
    """
    ...
    supported in channels and for messages sent in channel direct
...
class RichBlockExpandableBlockQuotation(BaseModel):
    """A block quotation, corresponding to the HTML tag <blockquote> with
...
class RichBlockButtons(BaseModel):
    ...
    align: str | None = None
    """Horizontal alignment of the buttons. Currently, must be one of
...
class InputRichBlockDocument(BaseModel):
    ...
    def _resolve(self, sink: _FileSink) -> dict[str, Any]:
        data = _dump(self, "document")
        data["document"] = self.document._resolve(sink)
        return data
...
class InputRichMessageContent(BaseModel):
    ...
    rich_message: InputRichMessage
    """The message to be sent. Only previously uploaded files may be used
    in the message.
    """
